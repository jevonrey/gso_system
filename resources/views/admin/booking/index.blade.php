            ...
            <tbody>
                @forelse($bookings as $booking)
                    <tr class="border-t">
                        ...
                        <td class="p-2">{{ $booking->requestor_name }}</td>
                        <td class="p-2">{{ \Carbon\Carbon::parse($booking->start_datetime)->format('M d Y, h:i A') }}
                        </td>
                        <td class="p-2">{{ \Carbon\Carbon::parse($booking->end_datetime)->format('M d Y, h:i A') }}</td>
                        <td
                            class="p-2 font-bold {{ $booking->status === 'Approved' ? 'text-green-700' : ($booking->status === 'Rejected' ? 'text-red-700' : 'text-yellow-600') }}">
                            {{ $booking->status }}</td>
                ...
                @empty
                    <tr>
                        <td colspan="5" class="text-center p-4">No bookings found.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
