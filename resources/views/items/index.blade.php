...
@section('content')
    <div class="d-flex justify-content-between mb-3">
        ...
        <a href="{{ route('items.create') }}" class="text-gray-300 bg-blue-700 text-lg px-3 py-1 rounded text-center">Add New
            Item</a><br>
    </div>
    <!-- Search Form -->
    <form method="GET" action="{{ route('items.index') }}" class="float-right">
        <input type="text" class="rounded-md p-2 border border-gray-300 text-gray-900" name="search"
            value="{{ request('search') }}" placeholder="Search items..." class="px-3 py-2 border rounded-md" />
                    ...
                    <td>
                        <div>
                            <a href="{{ route('items.edit', $item->id) }}"
                                class="bg-green-600 hover:bg-green-900 text-white text-xs rounded px-2">Edit</a><br>
